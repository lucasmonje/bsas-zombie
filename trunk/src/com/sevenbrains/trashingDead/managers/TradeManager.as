...
package com.sevenbrains.trashingDead.managers {
	...
	public class TradeManager {
		...
		public function processReward(value:ITradeValue, quantity:int = 1, context:IOperationContext = null, applyModifiers:Boolean = false):TradeValueInfo {
			if (!value) {
				return null;
			}
			...
			var values:Vector.<ITradeValue> = getRewardProcessor(value).preProcessReward(value, context, applyModifiers);
			var result:Vector.<ITradeValue> = new Vector.<ITradeValue>();
			...
			for (var i:int = 0; i < quantity; i++) {
				for each (v in values) {
					if (!LockManager.instance.isLocked(v.lock)) { // check lock in preprocessed value
						result = result.concat(getRewardProcessor(v).processReward(v, context));
					}
				}
			}
